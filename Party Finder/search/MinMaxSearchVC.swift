...
class MinMaxSearchVC: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func getData(uploadData: Data) {
        let urlStr = "http://localhost:8081/api/party/getAvailable"
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showParty" {
            if let vc = segue.destination as? FindVC {
